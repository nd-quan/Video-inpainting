...
class CrossAttnDownBlock2D(nn.Module):
    def __init__(
        ...
        #### 나은
        # -------------------------
        # BrushNet -> SPADE injection modules
        # -------------------------
        n_inject = num_layers + (1 if add_downsample else 0)
        self.brush_spades = nn.ModuleList(
            [SPADE(norm_nc=out_channels, label_nc=out_channels, config_text="spadeinstance3x3") for _ in range(n_inject)]
        )
    ...
    def forward(
        ...
        spades = list(self.brush_spades) if down_block_add_samples is not None else None
        ...
        for i, (resnet, attn) in enumerate(blocks):
            ...
            if down_block_add_samples is not None:
                ...
                ## 나은 spade
                cond = down_block_add_samples.pop(0)
                hidden_states = spades.pop(0)(hidden_states, cond)
            output_states = output_states + (hidden_states,)
...
class DownBlock2D(nn.Module):
    def __init__(
        ...
        self.gradient_checkpointing = False
        # -------------------------
        # BrushNet -> SPADE injection modules 나은
        # -------------------------
        n_inject = num_layers + (1 if add_downsample else 0)
        self.brush_spades = nn.ModuleList(
            [SPADE(norm_nc=out_channels, label_nc=out_channels, config_text="spadeinstance3x3") for _ in range(n_inject)]
...
class CrossAttnUpBlock2D(nn.Module):
    def __init__(
        ...
        # BrushNet -> SPADE injection modules
        # -------------------------
        n_inject = num_layers + (1 if add_upsample else 0)
        self.brush_spades = nn.ModuleList(
            [SPADE(norm_nc=out_channels, label_nc=out_channels, config_text="spadeinstance3x3") for _ in range(n_inject)]
        )
    ...
    def forward(
        ...
        spades = list(self.brush_spades) if up_block_add_samples is not None else None
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            ...
            if up_block_add_samples is not None:
                ...
                # 나은
                cond = up_block_add_samples.pop(0)
                hidden_states = spades.pop(0)(hidden_states, cond)
