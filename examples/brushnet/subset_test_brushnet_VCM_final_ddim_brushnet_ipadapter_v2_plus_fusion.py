# CUDA_VISIBLE_DEVICES=0 python /home/gpu_03/nas_naeun/BrushNet/examples/brushnet/subset_test_brushnet_VCM_final_ddim_brushnet_ipadapter_v2_plus_fusion.py
# conda activate brushnet_ipadapter

import os
from glob import glob
from tqdm import tqdm
from diffusers import StableDiffusionBrushNetPipeline, BrushNetModel, DDIMScheduler, UNet2DConditionModel
import torch
import cv2
import numpy as np
...
# 설정
image_dir = "/home/gpu_03/nas_naeun/data/data/test_in_COCO" # coco
...
mask_dir = "/home/gpu_03/nas_naeun/data/data/test_mask_COCO" # coco
...
caption_txt='/home/gpu_03/nas_naeun/data/data/caption/test/captions_test_COCO.txt' #coco(ws09)
# caption_txt="/media/ssd2/naeun/data/caption/train/COCO_train_caption.txt"
...
output_dir='/home/gpu_03/NAS_NE/Result/result/brushnet/COCO/ipadapter/check_v7_3b'
# test 15는 14에서 그냥 copy&paste
# test 16은 blending을 반대로 
os.makedirs(output_dir, exist_ok=True)

# 범위지정
selected_indices = [5, 44, 51, 88, 228, 394, 639, 644, 676, 775,788,847,1026,1109,1206,1215,1263,1366,1401,1467 ]   # 특정 번호 입력, 기본값 그냥빈간, 콤마로 구분
selected_range = None   # 범위, 기본값 None
...
brushnet_path="/home/gpu_03/naeun/v7/checkpoint-300000/brushnet"
# 블렌딩 설정 바꾸기
blended = True
...
ip_ckpt="/home/gpu_03/naeun/v7/checkpoint-300000/ipadapter/model.safetensors"
...
fusion_ckpt = "/home/gpu_03/naeun/v7/checkpoint-300000/ipadapter/fusion_module.safetensors"
...
if selected_indices is not None and len(selected_indices) > 0:
    target_idx_set.update(selected_indices)

if selected_range is not None:
    start_idx, end_idx = selected_range
    if start_idx > end_idx:
        raise ValueError(f"selected_range 시작값이 끝값보다 큼: {selected_range}")
    target_idx_set.update(range(start_idx, end_idx + 1))

if len(target_idx_set) == 0:
    indexed_selected = [
        (idx, img, msk, cap)
        for idx, (img, msk, cap) in indexed_all
    ]
    print(f"[Range] 전체 {len(indexed_selected)}개 사용")
else:
    max_valid_idx = len(indexed_all) - 1
    invalid_indices = sorted([i for i in target_idx_set if i < 0 or i > max_valid_idx])
    if len(invalid_indices) > 0:
        raise ValueError(f"유효하지 않은 인덱스가 있음: {invalid_indices} / valid range: 0 ~ {max_valid_idx}")
    ...
    print(f"[Range] 지정한 인덱스만 사용: {sorted(target_idx_set)} / 총 {len(indexed_selected)}개")

indexed_pending = [
    (idx, img, msk, cap)
    for idx, img, msk, cap in indexed_selected
