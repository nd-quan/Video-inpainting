# brushnet과 ipadapter 함께 훈련함 (이때 brushnet에 ipadapter는 연결되게) 이거 계속 실패함
...
from custom_dataset_v03 import CustomDataset
...
def main(args):
    ...
    brushnet,ip_adapter, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        brushnet,ip_adapter, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(ip_adapter,brushnet):
                ...
                if noise_scheduler.config.prediction_type == "epsilon":
                    target = noise
                    pred_x0 = (noisy_latents - sqrt_one_minus_alpha_t * model_pred) / sqrt_alpha_t
                elif noise_scheduler.config.prediction_type == "v_prediction":
                    target = noise_scheduler.get_velocity(latents, noise, timesteps)
                    pred_x0 = sqrt_alpha_t * noisy_latents - sqrt_one_minus_alpha_t * model_pred
                else:
                    raise ValueError(f"Unknown prediction type {noise_scheduler.config.prediction_type}")
                loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean") # original diffusion loss
                
                # reconstruction loss 추가
                lamda_rec=1
                rec_loss = F.l1_loss(pred_x0.float(), latents.float(), reduction="mean")
                loss = diff_loss + lambda_rec * rec_loss
