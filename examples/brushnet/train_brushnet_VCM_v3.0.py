...
def main(args):
    ...
    # 이 부분 코드 수정됨 (나은) 추가한 파라미터는 랜덤 초기화되게
    ...
    for n, p in unet.named_parameters():
        p.requires_grad = ("spade" in n.lower())
    ...
    # Optimizer creation
    brushnet_params = list(brushnet.parameters())
    unet_spade_params = [p for n, p in unet.named_parameters() if p.requires_grad]  # spade only

    params_to_optimize = brushnet_params + unet_spade_params
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(brushnet):
                ...
                model_pred = unet(
                    ...
                    timesteps,
                    encoder_hidden_states=encoder_hidden_states,
                    down_block_add_samples=[s.to(dtype=weight_dtype) for s in down_block_res_samples[::-1]],
                    mid_block_add_sample=mid_block_res_sample.to(dtype=weight_dtype),
                    up_block_add_samples=[s.to(dtype=weight_dtype) for s in up_block_res_samples[::-1]],
                    return_dict=False,
                )[0]
                ...
                if accelerator.sync_gradients:
                    params_to_clip = list(brushnet.parameters()) + [p for n,p in unet.named_parameters() if p.requires_grad]
    ...
    # Create the puneline using using the trained modules and save it.
