...
from custom_dataset_v02 import CustomDataset
...
from VCMCrossAttention import ContrastiveEmphasisVCM
...
def main(args):
    ...
    # v1.1 초기화
    # fuser = ContrastiveEmphasisVCM(in_ch=4, d_model=64, nhead=4,alpha_roi=1.0, beta_bg=1.0).to(accelerator.device)
    fuser = ContrastiveEmphasisVCM(in_ch=4, d_model=64, nhead=4).to(accelerator.device)
    ...
    if args.brushnet_model_name_or_path:
        ...
        brushnet = BrushNetModel.from_unet(unet,conditioning_channels=8)
    ...
    #         # ✅ BrushNetModel인 것만 골라서 저장
    #         brushnet_to_save = None
    #         for i, model in enumerate(models):
    #             m = unwrap_model(model)
    #             if isinstance(m, BrushNetModel):
    #                 brushnet_to_save = m
    #                 # weights와 models를 맞춰주기 위해 해당 weight 하나 제거
    #                 if i < len(weights):
    #                     weights.pop(i)
    #                 break

    #         if brushnet_to_save is not None:
    #             sub_dir = "brushnet"
    #             save_dir = os.path.join(output_dir, sub_dir)
    #             os.makedirs(save_dir, exist_ok=True)
    #             brushnet_to_save.save_pretrained(save_dir)

    #         # 나머지 weight들은 accelerate가 알아서 처리하지 않도록 모두 비워버리기
    #         while len(weights) > 0:
    #             weights.pop()   

    # def load_model_hook(models, input_dir):
    #     # ✅ BrushNetModel에만 저장된 weight를 로드
    #     idx_to_pop = None
    #     for i, model in enumerate(models):
    #         m = unwrap_model(model)
    #         if isinstance(m, BrushNetModel):
    ...
    #     # BrushNet은 pop해서 중복 로딩 방지
    #     if idx_to_pop is not None:
    #         models.pop(idx_to_pop)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        # 🔥 이 두 줄이 반드시 if 블록 안의 가장 마지막에 있어야 함
        accelerator.register_save_state_pre_hook(save_model_hook)
        accelerator.register_load_state_pre_hook(load_model_hook)
    ...
    # )
    # v1.1 NE
    params_to_optimize = list(brushnet.parameters()) + list(fuser.parameters())
    ...
    # )
    brushnet, fuser, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        brushnet, fuser, optimizer, train_dataloader, lr_scheduler
    )
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            # ✅ 2) fuser는 우리가 따로 저장했으므로 별도로 복구
            # NE v1.1
            try:
                fuser_ckpt = os.path.join(args.output_dir, path, "fuser.pt")
                if os.path.exists(fuser_ckpt):
                    # 래핑된 모듈은 unwrap해서 로드하는 것이 안전
                    accelerator.unwrap_model(fuser).load_state_dict(
                        torch.load(fuser_ckpt, map_location="cpu"), strict=False
                    )
                    logger.info(f"Loaded fuser weights from {fuser_ckpt}")
                else:
                    logger.warning("No fuser checkpoint found alongside accelerate state; using current fuser weights.")
            except Exception as e:
                logger.warning(f"Failed to load fuser weights: {e}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(brushnet):
                ...
                BG=vae.encode(batch["BG_condition"].to(dtype=weight_dtype)).latent_dist.sample()
                BG = BG * vae.config.scaling_factor
                
                ROI=vae.encode(batch["ROI_condition"].to(dtype=weight_dtype)).latent_dist.sample()
                ROI = ROI * vae.config.scaling_factor
                # ---- 디버깅용: fuser 전 상태를 보관
                BG_pre, ROI_pre = BG.detach(), ROI.detach()
                DEBUG_VIZ_STEPS = 2
                
                # NE v1.1 , v1.12
                BG, ROI = fuser(BG.to(weight_dtype), ROI.to(weight_dtype))
                ...
                #         latents.shape[-1]
                #     ),
                #     mode="nearest" # 0-1로 바꾸려 쓴다는데 잘모르겠음ㅋ
                # )
                
                # BG, ROI = fuser(BG.to(weight_dtype), ROI.to(weight_dtype),roi_mask=masks)
                
                # =========================================
                # 🔍 디버그 시각화 저장 (validation 없이, 초반 step만)
                # =========================================
                if accelerator.is_main_process and global_step < DEBUG_VIZ_STEPS:
                    os.makedirs(os.path.join(args.output_dir, "debug_feature_viz"), exist_ok=True)
                    step_dir = os.path.join(args.output_dir, "debug_feature_viz", f"step_{global_step:07d}")
                    os.makedirs(step_dir, exist_ok=True)

                    # 배치에서 1개 샘플만 저장 (필요하면 idx 바꿔)
                    idx = 0
                    bg_pre_1  = BG_pre[idx].detach().float().cpu()   # (4,64,64)
                    roi_pre_1 = ROI_pre[idx].detach().float().cpu()
                    bg_post_1  = BG[idx].detach().float().cpu()
                    roi_post_1 = ROI[idx].detach().float().cpu()

                    # --- (A) 채널별 히트맵 그리드 저장: 4채널을 한 줄로
                    def _grid4(x):
                        # x: (4,H,W) → (1,Hgrid,Wgrid) 그레이스케일 그리드
                        # 채널별 min-max 정규화
                        c, h, w = x.shape
                        x2 = x.reshape(c, -1)
                        x2 = (x2 - x2.min(dim=1, keepdim=True).values) / (x2.max(dim=1, keepdim=True).values - x2.min(dim=1, keepdim=True).values + 1e-8)
                        x2 = x2.reshape(c, h, w)
                        grid = make_grid(x2.unsqueeze(1), nrow=c, padding=2)  # (1,Hgrid,Wgrid)
                        return grid

                    save_image(_grid4(bg_pre_1),  os.path.join(step_dir, "BG_pre_latent_channels.png"))
                    save_image(_grid4(roi_pre_1), os.path.join(step_dir, "ROI_pre_latent_channels.png"))
                    save_image(_grid4(bg_post_1), os.path.join(step_dir, "BG_post_latent_channels.png"))
                    save_image(_grid4(roi_post_1),os.path.join(step_dir, "ROI_post_latent_channels.png"))
                    save_image(_grid4((bg_post_1 - bg_pre_1).abs()),  os.path.join(step_dir, "BG_delta_channels.png"))
                    save_image(_grid4((roi_post_1 - roi_pre_1).abs()), os.path.join(step_dir, "ROI_delta_channels.png"))

                    # --- (B) VAE 디코딩해서 사람이 보기 좋은 RGB로 저장 (1장씩)
                    with torch.no_grad(), torch.autocast("cuda", enabled=(accelerator.mixed_precision in ["fp16","bf16"])):
                        def _decode_rgb(lat):
                            # lat: (1,4,H,W) — 디코드할 때는 scaling_factor로 나눔
                            lat = lat.to(accelerator.device, dtype=weight_dtype) / vae.config.scaling_factor
                            img = vae.decode(lat).sample  # (1,3,H',W')
                            img = (img / 2 + 0.5).clamp(0, 1)
                            return img

                        save_image(_decode_rgb(BG_pre[idx:idx+1]),  os.path.join(step_dir, "BG_pre_decode.png"))
                        save_image(_decode_rgb(ROI_pre[idx:idx+1]), os.path.join(step_dir, "ROI_pre_decode.png"))
                        save_image(_decode_rgb(BG[idx:idx+1]),      os.path.join(step_dir, "BG_post_decode.png"))
                        save_image(_decode_rgb(ROI[idx:idx+1]),     os.path.join(step_dir, "ROI_post_decode.png"))

                    # --- (C) 완전 원시 텐서도 보관(.pt) → 나중에 노트북에서 상세 분석하기 좋음
                    torch.save({"BG_pre": BG_pre[idx].cpu(), "ROI_pre": ROI_pre[idx].cpu(),
                                "BG_post": BG[idx].cpu(), "ROI_post": ROI[idx].cpu()},
                            os.path.join(step_dir, "latents.pt"))
                
                
                # conditioning_latents=torch.concat([conditioning_latents,masks],1)
                conditioning_latents=torch.concat([BG,ROI],1)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        # === fuser 파라미터 따로 저장 ===
                        try:
                            fuser_to_save = accelerator.unwrap_model(fuser)
                            torch.save(fuser_to_save.state_dict(), os.path.join(save_path, "fuser.pt"))
                            logger.info(f"Saved fuser weights to {os.path.join(save_path, 'fuser.pt')}")
                        except Exception as e:
                            logger.warning(f"Failed to save fuser: {e}")
