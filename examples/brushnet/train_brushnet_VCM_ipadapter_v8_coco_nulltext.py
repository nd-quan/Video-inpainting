# V8 COCO null-text:
# - Train with the new local COCO_train dataset.
# - Do not use per-image text captions; always feed empty-string text embeddings.
# - Initialize BrushNet/IP-Adapter/FGBG fusion from checkpoint-200000.
# Based on V7, keeping the same model structure.
#!/usr/bin/env python
# coding=utf-8
# conda : diffusers
import argparse
...
DEFAULT_COCO_TRAIN_ROOT = "/media/ssd1/ndquan/model_naeun/paper/BrushNet/examples/brushnet/dataset/COCO_train"
DEFAULT_NULL_CAPTION_FILE = os.path.join(DEFAULT_COCO_TRAIN_ROOT, "caption_empty.txt")
DEFAULT_CHECKPOINT_200000 = "/media/ssd1/ndquan/model_naeun/paper/BrushNet/examples/checkpoint_naeun/checkpoint-200000"
...
DEFAULT_VALIDATION_IMAGE = os.path.join(DEFAULT_COCO_TRAIN_ROOT, "input", "000750.png")
DEFAULT_VALIDATION_MASK = os.path.join(DEFAULT_COCO_TRAIN_ROOT, "mask", "000750.png")
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=DEFAULT_BRUSHNET_200000,
        help="Path to pretrained brushnet model or model identifier from huggingface.co/models."
        " Defaults to the local checkpoint-200000 BrushNet. If not specified brushnet weights are initialized from unet.",
    )
    parser.add_argument(
        "--coco_train_root",
        type=str,
        default=DEFAULT_COCO_TRAIN_ROOT,
        help="Root folder of the local COCO_train dataset. Expected subfolders: gt, input, mask.",
    )
    parser.add_argument(
        "--null_caption_file",
        type=str,
        default=DEFAULT_NULL_CAPTION_FILE,
        help=(
    ...
    type=str,
    default=DEFAULT_IPADAPTER_200000,
    help="Path to pretrained ip-adapter checkpoint (optional)."
        ),
    parser.add_argument(
    "--fusion_module_path",
    type=str,
    default=DEFAULT_FUSION_200000,
...
def main(args):
    ...
    train_dataset = CustomDataset(
        ...
        tokenizer=tokenizer,
        # V8 is image-guidance/null-text training. Keep image conditioning active
        # for every sample. With DDP and batch_size=1 per GPU, stochastic image
        # dropout can make the fusion branch unused on only some ranks, which may
        # hang NCCL all-reduce/broadcast.
        t_drop_rate=0.0,
        i_drop_rate=0.0,
        ti_drop_rate=0.0,
    )
