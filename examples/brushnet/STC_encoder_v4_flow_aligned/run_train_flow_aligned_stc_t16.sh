...
TRAIN_SCRIPT="${SCRIPT_DIR}/train_flow_aligned_stc_shared_noise.py"
...
INIT_STC_ADAPTER="${INIT_STC_ADAPTER:-${BRUSHNET_ROOT}/experiments/checkpoint_rgb_stc_v2_sharedNoise_0.9/checkpoint-2000/stc_adapter}"
OUTPUT_DIR="${OUTPUT_DIR:-${BRUSHNET_ROOT}/experiments/train_stc_v4_flow_aligned_T16_S12_sharedNoise_0.9}"
...
if [[ -n "${INIT_STC_ADAPTER}" ]]; then
    REQUIRED_PATHS+=("${INIT_STC_ADAPTER}/config.json")
fi
...
COMMON_ARGS=(
    ...
    --tracker_project_name train_stc_v4_flow_aligned_t16
)
if [[ -n "${INIT_STC_ADAPTER}" ]]; then
...
echo "Flow-aligned RGB-STC: warm-start T8 -> fine-tune T16"
echo "clip=${CLIP_LENGTH}, stride=${CLIP_STRIDE}, flow_region=${FLOW_REGION}"
