...
LARGE_MOTION_THRESHOLD="${LARGE_MOTION_THRESHOLD:-0.5}"
MOTION_LOSS_REGION="${MOTION_LOSS_REGION:-bg}"
LARGE_MOTION_LOSS_WEIGHT="${LARGE_MOTION_LOSS_WEIGHT:-0.0}"
DIRECTION_LOSS_WEIGHT="${DIRECTION_LOSS_WEIGHT:-0.0}"
MAGNITUDE_LOSS_WEIGHT="${MAGNITUDE_LOSS_WEIGHT:-0.0}"
DIRECTION_NORM_EPS="${DIRECTION_NORM_EPS:-0.05}"
BEST_METRIC="${BEST_METRIC:-auto}"
BEST_ALL_EPE_WEIGHT="${BEST_ALL_EPE_WEIGHT:-0.25}"
...
common_args=(
  ...
  --motion_loss_region "${MOTION_LOSS_REGION}"
  --large_motion_loss_weight "${LARGE_MOTION_LOSS_WEIGHT}"
  --direction_loss_weight "${DIRECTION_LOSS_WEIGHT}"
  --magnitude_loss_weight "${MAGNITUDE_LOSS_WEIGHT}"
  --direction_norm_eps "${DIRECTION_NORM_EPS}"
  --best_metric "${BEST_METRIC}"
  --best_all_epe_weight "${BEST_ALL_EPE_WEIGHT}"
...
echo "objective=all Charbonnier + ${LARGE_MOTION_LOSS_WEIGHT}*large-motion Charbonnier + ${DIRECTION_LOSS_WEIGHT}*direction + ${MAGNITUDE_LOSS_WEIGHT}*relative-magnitude"
echo "large_motion_threshold=${LARGE_MOTION_THRESHOLD} feature px; motion_loss_region=${MOTION_LOSS_REGION}; direction_norm_eps=${DIRECTION_NORM_EPS}; best_metric=${BEST_METRIC}"
