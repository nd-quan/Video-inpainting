    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(brushnet):
                ...
                ############### ROI/BG를 분리 나은
                ROI_latents = conditioning_latents * masks
                BG_latents = conditioning_latents * (1- masks)
                
                ############### ROI/BG decoder에 다시 통과
                def decode_latents_to_numpy(latents, vae):
                    # latent 스케일 되돌리기
                    latents = 1 / vae.config.scaling_factor * latents
                    # VAE 디코더 통과 ([-1,1] 범위 이미지)
                ...
                cond_latent_images = decode_latents_to_numpy(conditioning_latents, vae)
                roi_images = decode_latents_to_numpy(ROI_latents,vae)
                bg_images  = decode_latents_to_numpy(BG_latents,vae)
                
                ############### 시각화
                DEBUG_VIZ_STEPS = 5

                if accelerator.is_main_process and global_step < DEBUG_VIZ_STEPS:
                    save_dir = "./Debug/debug_latents"
                    os.makedirs(save_dir, exist_ok=True)

                    # 1) conditioning_pixel_values (모델 입력 이미지)
                    cond = batch["conditioning_pixel_values"]  # (B, C, H, W), 보통 [-1,1]
                    cond = (cond / 2 + 0.5).clamp(0, 1)
                    cond = cond.detach().cpu().permute(0, 2, 3, 1).float().numpy()  # (B,H,W,C)

                    # 2) mask (1채널 → 3채널로 만들어서 흑/백)
                    mask = batch["masks"]  # (B,1,H,W) 가정
                    mask = F.interpolate(mask, size=cond.shape[1:3])  # (B,1,H,W)
                    mask = mask.detach().cpu().squeeze(1).numpy()     # (B,H,W)
                    mask_rgb = np.stack([mask, mask, mask], axis=-1)  # (B,H,W,3)

                    batch_size = cond.shape[0]
                    num_to_save = min(batch_size, 5)

                    for i in range(num_to_save):
                        # numpy → uint8
                        cond_img     = (cond[i]              * 255).astype("uint8")
                        mask_img     = (mask_rgb[i]         * 255).astype("uint8")
                        cond_lat_img = (cond_latent_images[i] * 255).astype("uint8")
                        roi_img      = (roi_images[i]       * 255).astype("uint8")
                        bg_img       = (bg_images[i]        * 255).astype("uint8")

                        # numpy → PIL.Image (리사이즈는 안 함)
                        pil_imgs = [
                            Image.fromarray(cond_img),
                            Image.fromarray(mask_img),
                            Image.fromarray(cond_lat_img),
                            Image.fromarray(roi_img),
                            Image.fromarray(bg_img),
                        ]

                        # 캔버스 크기: 가로는 각 이미지 width 합, 세로는 높이 중 최대값
                        ...
                        x_offset = 0
                        for im_pil in pil_imgs:
                            # 세로는 위쪽 정렬로 붙이고, 이미지 높이가 더 작으면 아래는 까만 배경으로 남음
                            canvas.paste(im_pil, (x_offset, 0))
                            x_offset += im_pil.width

                        filename = os.path.join(
                            save_dir,
                            f"step{global_step:05}_sample{i:02}.png"
                        )
