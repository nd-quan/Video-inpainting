...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/experiments/train_dgaf_original_${WARP_MODE}_conf${USE_CONFIDENCE_MASK}_T3_S1}"
ARGS=(--warp_mode "$WARP_MODE" "$CONFIDENCE_ARG" --output_dir "$OUTPUT_DIR"
      --upscale_factor "${UPSCALE_FACTOR:-4}"
      --clip_length "${CLIP_LENGTH:-3}" --clip_stride "${CLIP_STRIDE:-1}"
      ...
      --shared_bg_noise_strength "${SHARED_BG_NOISE_STRENGTH:-0.0}")
