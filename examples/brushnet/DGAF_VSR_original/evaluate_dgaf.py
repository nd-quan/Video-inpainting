...
from DGAF_VSR_original.common import add_common_arguments, resolve_paths, make_sequence_dataset, load_stack, json_dump, BooleanOptionalAction
from DGAF_VSR_original.warping import SequenceWarper, GuidanceConfig


def parse_args(argv=None):
    parser = argparse.ArgumentParser(description=__doc__)
    add_common_arguments(parser)
    parser.set_defaults(split="test", shared_bg_noise_strength=None, fusion_scale=None)
    ...
    parser.add_argument("--max_clips", type=int, default=None, help="Maximum complete contiguous sequences")
    ...
    parser.add_argument("--save_auxiliary", action=BooleanOptionalAction, default=True,
                        help="Save raw and input PNGs in addition to final and GT")
...
def main(args):
    ...
    if contract.get("schema_version") != 2:
        raise ValueError("This evaluator requires an original-style v2 training contract")
    ...
    for key in ("shared_bg_noise_strength", "fusion_scale"):
        if getattr(args, key) is None:
            setattr(args, key, contract[key])
    ...
    dataset = make_sequence_dataset(args)
    ...
    json_dump(args.output_dir / "model_contract.json", {**record, "ip_loading": ip_report,
        "stc_encoder": False, "memory_reset": "every contiguous sequence", "sampling": "sequential_same_timestep_alternating",
    ...
    with torch.inference_mode():
        for index in range(limit):
            ...
            # Bound encoder activations independently of sequence length.
            embeddings = []
            for frame in range(len(ids)):
                frame_sample = {key: value[frame:frame+1] if torch.is_tensor(value) else value
                                for key, value in sample.items()}
                embeddings.append(build_v8_prompt_embeddings(
                    pipe, image_encoder, projection, fusion, frame_sample, device, args.fusion_scale))
            prompt, negative, text, negative_text = [torch.cat(parts) for parts in zip(*embeddings)]
            ...
            warper = SequenceWarper(flows.forward, flows.backward, bg[None], config)
    ...
    summary["aggregation"] = "unweighted contiguous-sequence mean; each frame emitted once"
