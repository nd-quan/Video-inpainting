...
from DGAF_VSR_original.warping import GuidanceConfig
...
def add_common_arguments(parser):
    ...
    parser.add_argument("--shared_bg_noise_strength", type=float, default=0.0)
...
def add_guidance_arguments(parser):
    ...
    parser.add_argument("--direction", choices=("alternating", "previous", "next"), default="alternating")
...
def load_stack(args, device, *, model_path=None, inference=False):
    ...
    # Baseline restoration is the frozen denoiser for source estimates/boundaries.
    pipe.baseline_brushnet = BrushNetModel.from_pretrained(str(args.baseline_checkpoint / "brushnet"))
    pipe.baseline_brushnet.to(device=device, dtype=dtype).requires_grad_(False).eval()
...
def make_sequence_dataset(args):
    """Recover each complete contiguous run using all adjacent pairs, no overlap output."""
    import copy
    scan = copy.copy(args)
    scan.clip_length, scan.clip_stride = 2, 1
    dataset = make_dataset(scan)
    if getattr(dataset, "covered_frame_count", None) != getattr(dataset, "frame_count", None):
        raise ValueError("Sequence indexing found isolated frames without an adjacent pair; split or repair these runs before evaluation")
    grouped = {}
    for branch, paths, ids in dataset.clips:
        frames = grouped.setdefault(branch, {})
        for path, frame_id in zip(paths, ids):
            if frame_id in frames and frames[frame_id] != path:
                raise ValueError("Conflicting sequence frame paths")
            frames[frame_id] = path
    sequences = []
    for branch, frames in sorted(grouped.items()):
        run = []
        for frame_id, path in sorted(frames.items()):
            if run and frame_id != run[-1][0] + 1:
                sequences.append((branch, tuple(p for _, p in run), tuple(i for i, _ in run)))
                run = []
            run.append((frame_id, path))
        if run:
            sequences.append((branch, tuple(p for _, p in run), tuple(i for i, _ in run)))
    dataset.clips = sequences
    return SequenceDataset(dataset)


class SequenceDataset:
    def __init__(self, dataset):
        self.dataset = dataset
        self.clips = dataset.clips

    def __len__(self):
        return len(self.clips)

    def __getitem__(self, index):
        # Underlying dataset uses clip_length to repeat null text/drop flags.
        self.dataset.clip_length = len(self.clips[index][2])
