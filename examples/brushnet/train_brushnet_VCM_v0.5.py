...
def count_params(model, name="model"):
    ...
    print(f"[{name}] total params: {total/1e6:.2f}M, trainable: {trainable/1e6:.2f}M")
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--pixel_loss_weight",type=float,default=0.1,help="Weight for image-domain pixel L2 loss.",) #나은
...
def main(args):
    ...
    count_params(text_encoder, "Text Encoder")
    count_params(brushnet, "BrushNet")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(brushnet):
                ...
                # ======== Simple pixel loss (no mask, image domain) ========
                # pred x0 (latent) 복원
                with torch.no_grad():
                    alpha_bar_t = noise_scheduler.alphas_cumprod[timesteps]  # (B,)
                    while alpha_bar_t.ndim < noisy_latents.ndim:
                        alpha_bar_t = alpha_bar_t.view(-1, *([1] * (noisy_latents.ndim - 1)))

                    if noise_scheduler.config.prediction_type == "epsilon":
                        x0_latent_pred = (noisy_latents - torch.sqrt(1 - alpha_bar_t) * model_pred) / torch.sqrt(alpha_bar_t)
                    elif noise_scheduler.config.prediction_type == "v_prediction":
                        x0_latent_pred = (
                            torch.sqrt(alpha_bar_t) * noisy_latents
                            - torch.sqrt(1 - alpha_bar_t) * model_pred
                        )
                    else:
                        raise ValueError(f"Unsupported prediction_type {noise_scheduler.config.prediction_type}")

                # image 도메인으로 디코딩
                    x0_img_pred = vae.decode(x0_latent_pred / vae.config.scaling_factor).sample  # (B,3,H,W), [-1,1]

                # GT 준비
                    gt_img = batch["pixel_values"].to(dtype=x0_img_pred.dtype)  # (B,3,H,W), [-1,1]

                # 전체 픽셀 평균 L2
                lpixel = F.mse_loss(x0_img_pred, gt_img)
                
                loss = noise_loss + args.pixel_loss_weight * lpixel
            ...
            logs = {
                "loss_total": loss.detach().item(),          # 최종 loss
                "loss_noise": noise_loss.detach().item(),    # diffusion noise loss
                "loss_pixel": lpixel.detach().item(),        # 이미지 L2 loss
