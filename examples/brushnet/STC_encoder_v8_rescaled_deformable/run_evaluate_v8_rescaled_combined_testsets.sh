...
EVAL_SCRIPT="${SCRIPT_DIR}/evaluate_v8_rescaled_deformable.py"
...
OUTPUT_DIR="${OUTPUT_DIR:-${BRUSHNET_ROOT}/experiments/eval_stc_v8_rescaled_x4/checkpoint-${CHECKPOINT_STEP}-combined-testsets}"
...
if [[ -f "${RAFT_STUDENT_PATH}" ]]; then
    grep -q '"checkpoint"' "${RAFT_STUDENT_PATH}" || {
        echo "Invalid V7 RAFT pointer: ${RAFT_STUDENT_PATH}" >&2
        exit 1
    }
elif [[ ! -f "${RAFT_STUDENT_PATH}/raft_student/pytorch_model.bin" \
     && ! -f "${RAFT_STUDENT_PATH}/pytorch_model.bin" ]]; then
    echo "Missing V7 RAFT student: ${RAFT_STUDENT_PATH}" >&2
    exit 1
fi
...
if (( ${#duplicates[@]} > 0 )); then
    echo "Duplicate sequence names prevent one output root: ${duplicates[*]}" >&2
    exit 2
fi
...
echo "V8-R evaluation: checkpoint=${CHECKPOINT_STEP}, model=${STC_V8R_MODEL}"
echo "RAFT=${RAFT_STUDENT_PATH}, T=${CLIP_LENGTH}, S=${CLIP_STRIDE}, deform=${DEFORMABLE_ALIGNMENT_SCALE}"
echo "CGE=off, temporal DDIM guidance=off, output=${OUTPUT_DIR}"
...
for index in "${!roots[@]}"; do
    ...
    if [[ "${RUN_PREFLIGHT}" == "1" ]]; then
        "${PYTHON_BIN}" "${EVAL_SCRIPT}" "${COMMON_ARGS[@]}" \
            --dataset_root "${root}" --preflight_only "$@"
    fi
    "${PYTHON_BIN}" "${EVAL_SCRIPT}" "${COMMON_ARGS[@]}" \
        --dataset_root "${root}" "$@"
...
echo "Combined V8-R evaluation complete: ${OUTPUT_DIR}/summary.json"
