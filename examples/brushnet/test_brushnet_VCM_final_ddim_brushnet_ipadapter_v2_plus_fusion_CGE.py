...
output_dir='/media/ssd1/ndquan/model_naeun/paper/BrushNet/Quan_test/results/Generated_image/PartyScene_long/new/sharedNoise_mean_32_guided_v0_PGD'
...
shared_clip_size = int(os.environ.get("CGE_SHARED_CLIP_SIZE", os.environ.get("CGE_CLIP_SIZE", "64")))
...
clips = [indexed_all[i:i + shared_clip_size] for i in range(0, len(indexed_all), shared_clip_size)]
clips_to_run = [
    (clip_idx, clip)
...
print(
    f"[CGE + PGD shared-noise] shared_clip_size={shared_clip_size}, "
    f"run_batch_size={run_batch_size}, clips_to_run={len(clips_to_run)}, "
    f"pgd_window=[{pgd_start_step}, {pgd_end_step}), pgd_steps={pgd_steps}, pgd_lr={pgd_lr}"
)
