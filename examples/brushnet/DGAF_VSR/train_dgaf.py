#!/usr/bin/env python
"""Train STC-free BrushNet using detached short DDIM rollouts for guidance.

GT is used only for forward noising and the restoration loss target. Temporal
guidance always comes from the online model's predicted clean latent. No GT
latent or GT/teacher flow is injected as a temporal condition.
"""
import argparse
import json
import math
...
from DGAF_VSR.common import (add_common_arguments, add_guidance_arguments, guidance_from_args,
    resolve_paths, make_dataset, dataset_fingerprint, load_stack, json_dump, BooleanOptionalAction)
from DGAF_VSR.warping import ClipWarper, rollout_target
...
def parse_args(argv=None):
    ...
    parser.add_argument("--rollout_steps", type=int, default=1,
                        help="Detached DDIM warmup steps before the differentiable guided step")
    parser.add_argument("--bootstrap_probability", type=float, default=0.1,
                        help="Train without cached guidance at the first sampling timestep")
    ...
    for name in ("train_batch_size", "gradient_accumulation_steps", "max_train_steps", "checkpointing_steps", "num_diffusion_steps", "rollout_steps"):
        if getattr(args, name) < 1:
            parser.error(f"{name} must be positive")
    if not 0 <= args.bootstrap_probability < 1 or args.rollout_steps >= args.num_diffusion_steps:
        parser.error("Require 0 <= bootstrap_probability < 1 and rollout_steps < num_diffusion_steps")
    ...
    if args.smoke_test:
        ...
        args.bootstrap_probability = 0.0
...
def training_contract(args, dataset, num_processes):
    ...
    values.update(num_processes=num_processes, dataset_fingerprint=dataset_fingerprint(dataset),
        schema_version=1, stc_encoder=False, trainable="brushnet_all",
        guidance_source="detached_online_pred_original_sample_previous_step",
        guidance_update="synchronous_clip_snapshot", training_method="truncated_DDIM_rollout_effective_residual_MSE",
        ...
        for batch_index, batch in enumerate(active_loader, start=skip_batches):
            with accelerator.accumulate(model):
                ...
                with torch.no_grad():
                    ...
                    warper = ClipWarper(flows.forward, flows.backward, bg.reshape(b, frames, 1, h, w), guidance_from_args(args))
                    ...
                    bootstrap = bool(torch.rand((), device=accelerator.device) < args.bootstrap_probability)
                    index = 0 if bootstrap else int(torch.randint(args.rollout_steps, args.num_diffusion_steps, (), device=accelerator.device))
                    first = index if bootstrap else index - args.rollout_steps
                    timestep = scheduler.timesteps[first]
                    state = scheduler.add_noise(clean_gt, noise, timestep.expand(b * frames))
                    cached = None
                    # Truncated on-policy rollout: model-generated state and clean
                    # memory, detached together. No full-trajectory backpropagation.
                    for warm_index in range(first, index):
                        guidance = warper.guidance(cached, warm_index).flatten(0, 1)
                        condition = torch.cat((base_condition, guidance), dim=1)
                        with accelerator.autocast():
                            prediction = frozen_v8_predict(accelerator.unwrap_model(model), pipe.unet,
                                state.to(dtype), scheduler.timesteps[warm_index], condition.to(dtype), text, context)
                        transition = scheduler.step(prediction.float(), scheduler.timesteps[warm_index], state.float(), eta=0.0)
                        state = transition.prev_sample.detach()
                        cached = transition.pred_original_sample.detach().reshape(b, frames, 4, h, w)
                    guidance = warper.guidance(cached, index).flatten(0, 1)
                    condition = torch.cat((base_condition, guidance), dim=1)
                    timestep = scheduler.timesteps[index]
                    target = rollout_target(state, clean_gt, scheduler.alphas_cumprod[timestep], scheduler.config.prediction_type)
                with accelerator.autocast():
            ...
            if accelerator.sync_gradients and not overflow:
                ...
                metrics = {"loss": float(accelerator.gather(loss.detach()).mean()), "temporal_input_grad_norm": temporal_gradient,
                    ...
                    "global_step": step, "timestep": int(timestep), "elapsed_seconds": time.monotonic() - start}
