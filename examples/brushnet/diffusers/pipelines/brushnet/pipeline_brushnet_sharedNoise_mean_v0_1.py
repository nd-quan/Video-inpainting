...
class StableDiffusionBrushNetPipeline(
    ...
    def _encode_vae_in_chunks(self, image: torch.FloatTensor, chunk_size: int) -> torch.FloatTensor:
        if chunk_size is None or chunk_size <= 0 or chunk_size >= image.shape[0]:
        ...
        for start in range(0, image.shape[0], chunk_size):
            image_chunk = image[start:start + chunk_size]
            encoded.append(self.vae.encode(image_chunk).latent_dist.sample() * self.vae.config.scaling_factor)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if use_shared_bg_noise or shared_bg_noise is not None:
            ...
            print("bg_mix min/max/mean:", bg_mix.min().item(), bg_mix.max().item(), bg_mix.mean().item())
        ...
        vae_encode_chunk_size = int(os.environ.get("BRUSHNET_VAE_ENCODE_CHUNK_SIZE", "4"))
        conditioning_latents = self._encode_vae_in_chunks(image, vae_encode_chunk_size)
