...
class StableDiffusionBrushNetPipeline(
    ...
    def __call__(
        ...
        brushnet_condition: Optional[torch.FloatTensor] = None,
        brushnet_prompt_embeds: Optional[torch.FloatTensor] = None,
        ...
        **kwargs,
        ...
        # V8 training feeds text-only states to BrushNet and appends IP image
        # tokens only for the U-Net.  Keep the historical default for existing
        # callers, while allowing RGB-STC evaluation to provide that exact
        # text-only context explicitly.
        if brushnet_prompt_embeds is None:
            brushnet_context = prompt_embeds
        else:
            if (
                brushnet_prompt_embeds.ndim != 3
                or brushnet_prompt_embeds.shape[0] != prompt_embeds.shape[0]
                or brushnet_prompt_embeds.shape[-1] != prompt_embeds.shape[-1]
            ):
                raise ValueError(
                    "brushnet_prompt_embeds must be [prepared_batch, tokens, "
                    "cross_attention_dim] and match prompt_embeds batch/dim; "
                    f"got {tuple(brushnet_prompt_embeds.shape)} versus "
                    f"{tuple(prompt_embeds.shape)}"
                )
            brushnet_context = brushnet_prompt_embeds.to(
        ...
        if use_shared_bg_noise or shared_bg_noise is not None:
            ...
            if shared_bg_noise is None:
                ...
                if shared_bg_noise.shape[1] != noise.shape[1]:
                    raise ValueError(
                        f"shared_bg_noise must have {noise.shape[1]} channels, "
                        f"but got {shared_bg_noise.shape[1]}."
                    )
                if shared_bg_noise.shape[-2:] != noise.shape[-2:]:
                    shared_bg_noise = torch.nn.functional.interpolate(
                        shared_bg_noise,
                        size=noise.shape[-2:],
                        mode="nearest",
                    )
        ...
        # 6.1 Prepare the five-channel BrushNet condition. RGB-STC inference
        # may provide a precomputed [z_condition + delta_z_BG, M_BG] tensor;
        # the standard image/mask path remains exactly unchanged otherwise.
        if brushnet_condition is None:
            conditioning_latents = (
                self.vae.encode(image).latent_dist.sample()
            ...
            expected_shape = (
                image.shape[0],
                brushnet.config.conditioning_channels,
                height // self.vae_scale_factor,
                width // self.vae_scale_factor,
            )
            if tuple(brushnet_condition.shape) != expected_shape:
                raise ValueError(
                    "brushnet_condition must match the prepared BrushNet "
                    f"batch/layout; expected {expected_shape}, got "
                    f"{tuple(brushnet_condition.shape)}"
                )
            conditioning_latents = brushnet_condition.to(
                device=device, dtype=brushnet.dtype
            )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if guess_mode and self.do_classifier_free_guidance:
                    ...
                    brushnet_encoder_hidden_states = brushnet_context.chunk(2)[1]
                else:
                    control_model_input = latent_model_input
                    brushnet_encoder_hidden_states = brushnet_context
                ...
                down_block_res_samples, mid_block_res_sample, up_block_res_samples = self.brushnet(
                    control_model_input,
                    t,
                    encoder_hidden_states=brushnet_encoder_hidden_states,
