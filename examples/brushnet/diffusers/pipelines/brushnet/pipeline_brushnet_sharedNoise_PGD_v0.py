...
class StableDiffusionBrushNetPipeline(
    ...
    # for PGD background residual
    @staticmethod
    def _project_l2_ball(tensor: torch.FloatTensor, radius: float) -> torch.FloatTensor:
        if radius <= 0:
    ...
    def _prepare_shared_bg_noise(
        self,
        shared_bg_noise: Optional[torch.FloatTensor],
        reference_noise: torch.FloatTensor,
        device: torch.device,
        dtype: torch.dtype,
    ) -> torch.FloatTensor:
        if shared_bg_noise is None:
            return reference_noise.mean(dim=0, keepdim=True) * math.sqrt(reference_noise.shape[0])

        shared_bg_noise = shared_bg_noise.to(device=device, dtype=dtype)
        if shared_bg_noise.ndim == 3:
            shared_bg_noise = shared_bg_noise.unsqueeze(0)
        if shared_bg_noise.shape[1] != reference_noise.shape[1]:
            raise ValueError(
                f"shared_bg_noise must have {reference_noise.shape[1]} channels, "
                f"but got {shared_bg_noise.shape[1]}."
            )
        if shared_bg_noise.shape[-2:] != reference_noise.shape[-2:]:
            shared_bg_noise = F.interpolate(shared_bg_noise, size=reference_noise.shape[-2:], mode="nearest")
        return shared_bg_noise[:1]
    ...
    def __call__(
        ...
        #  shared background noise args
        use_shared_bg_noise: bool = False,
        shared_bg_noise: Optional[torch.FloatTensor] = None,
        shared_bg_noise_strength: float = 1.0,

        # pgd background residual args
        use_pgd_bg_residual: bool = False,
        pgd_steps: int = 5,
        pgd_start_step: int = 0,
        pgd_end_step: Optional[int] = None,
        pgd_lr: float = 1e-2,
        ...
        **kwargs,
        ...
        self.scheduler.use_pgd_bg_residual = False
        self.scheduler.last_cge_raw_grad = None

        #  Version 2 shared-noise prior:
        # z_hat = z_roi * ROI + (z_shared_bg,k + r_bg) * BG.
        # In this pipeline, original_mask is 1 on BG and 0 on ROI after binarization.
        if use_pgd_bg_residual or use_shared_bg_noise or shared_bg_noise is not None:
            latent_batch = latents.shape[0]
            bg_mask_latent = F.interpolate(
            ...
            shared_bg_noise_strength = max(0.0, min(1.0, float(shared_bg_noise_strength)))
            shared_bg_noise_mode = "clip_mean_shared_bg" if shared_bg_noise is None else "fixed_shared_bg"
            z_shared_bg = self._prepare_shared_bg_noise(shared_bg_noise, noise, device, latents.dtype).expand_as(noise)

            if use_pgd_bg_residual:
                residual_bg = (noise - z_shared_bg) * float(residual_init_scale)
                residual_bg = residual_bg * bg_mask_latent
                residual_bg = self._project_l2_ball(residual_bg, float(residual_sigma)) * bg_mask_latent
                bg_seed = z_shared_bg + residual_bg

                residual_norm = torch.linalg.vector_norm(
                    residual_bg.reshape(residual_bg.shape[0], -1).float(), ord=2, dim=1
                )
                self._shared_noise_pgd_stats = {
                    "mode": f"{shared_bg_noise_mode}_with_pgd_residual",
                    "batch": int(latent_batch),
                    "pgd_steps": int(pgd_steps),
                    "pgd_start_step": int(pgd_start_step),
                    "pgd_end_step": None if pgd_end_step is None else int(pgd_end_step),
                    "pgd_lr": float(pgd_lr),
                    "residual_sigma": float(residual_sigma),
                    "residual_norm_mean": float(residual_norm.mean().detach().cpu()),
                    "residual_norm_max": float(residual_norm.max().detach().cpu()),
                    "shared_bg_noise_strength": float(shared_bg_noise_strength),
                }

                self.scheduler.use_pgd_bg_residual = True
                self.scheduler.pgd_steps = int(pgd_steps)
                self.scheduler.pgd_step_count = 0
                self.scheduler.pgd_denoise_step_count = 0
                self.scheduler.pgd_start_step = int(pgd_start_step)
                self.scheduler.pgd_end_step = None if pgd_end_step is None else int(pgd_end_step)
                self.scheduler.pgd_lr = float(pgd_lr)
                self.scheduler.pgd_residual_sigma = float(residual_sigma)
                self.scheduler.pgd_latent_scale = float(self.scheduler.init_noise_sigma)
                self.scheduler.pgd_bg_mask_latent = bg_mask_latent.detach().clone()
                self.scheduler.pgd_residual_bg = residual_bg.detach().clone()
            else:
                bg_seed = z_shared_bg
                self._shared_noise_pgd_stats = {
                    "mode": f"{shared_bg_noise_mode}_without_residual",
                    "batch": int(latent_batch),
                    "shared_bg_noise_strength": float(shared_bg_noise_strength),
                }

            bg_mix = bg_mask_latent * shared_bg_noise_strength
            noise = noise * (1.0 - bg_mix) + bg_seed * bg_mix
            latents = noise * self.scheduler.init_noise_sigma
        ...
        if use_pgd_bg_residual and hasattr(self.scheduler, "pgd_step_count"):
            self._shared_noise_pgd_stats.update(
                {
                    "pgd_updates_applied": int(getattr(self.scheduler, "pgd_step_count", 0)),
                    "pgd_denoise_steps_seen": int(getattr(self.scheduler, "pgd_denoise_step_count", 0)),
                    "residual_norm_mean_final": float(getattr(self.scheduler, "pgd_residual_norm_mean", 0.0)),
                    "residual_norm_max_final": float(getattr(self.scheduler, "pgd_residual_norm_max", 0.0)),
                }
            )
...
class StableDiffusionBrushNetPipeline_v02(
    DiffusionPipeline,
    StableDiffusionMixin,
    TextualInversionLoaderMixin,
    LoraLoaderMixin,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if isinstance(brushnet, BrushNetModel):
            ...
            # NE수정(나은) make BG & ROI
            # original_mask=(original_mask.sum(1)[:,None,:,:] < 0).to(image.dtype)
            # mask = (original_mask.mean(1, keepdim=True) + 1.0) * 0.5
            # ROI = image * mask
            # BG=image*(1-mask)
            mask01 = (original_mask.mean(1, keepdim=True) + 1.0) * 0.5

            # Dataset과 동일: transforms.Lambda(lambda x: 1.0 - x)
            # => BG 마스크가 1이 되도록 뒤집기
            mask_bg  = 1.0 - mask01        # BG=1, ROI=0
            mask_roi = 1.0 - mask_bg       # ROI=1, BG=0

            # 이미지 공간에서 ROI/BG 분리 (Dataset과 동일 공식을 그대로)
            BG  = image * mask_bg          # synth*mask
            ROI = image * (1.0 - mask_bg)  # synth*(1-mask)
            height, width = image.shape[-2:]
        else:
            assert False
        ...
        # conditioning_latents = torch.concat([conditioning_latents,mask],1)
        BG=self.vae.encode(BG).latent_dist.sample() * self.vae.config.scaling_factor
        ROI=self.vae.encode(ROI).latent_dist.sample() * self.vae.config.scaling_factor
        
        # 6.2 prepare condition latents+fuser (NE)v1.1, v1.2(나은)
        # 🔥 fuser 적용 (있을 때만)
        if hasattr(self, "fuser") and self.fuser is not None:
            BG, ROI = self.fuser(BG, ROI)
        # v0.2    
        ...
        conditioning_latents=torch.concat([BG,ROI,mask],1)
