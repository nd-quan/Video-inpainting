...
EVAL_SCRIPT="${SCRIPT_DIR}/evaluate_v6_flow_deformable.py"
...
OUTPUT_DIR="${OUTPUT_DIR:-${BRUSHNET_ROOT}/experiments/eval_stc_v6_deformable/checkpoint-${CHECKPOINT_STEP}-combined-testsets}"
...
for required in \
    ...
    "${STC_V6_MODEL}/config.json" \
    "${STC_V6_MODEL}/diffusion_pytorch_model.safetensors" \
...
echo "RGB-STC V6 combined test-set evaluation"
echo "checkpoint=${CHECKPOINT_PATH}"
echo "stc_v6_model=${STC_V6_MODEL}"
echo "T=${CLIP_LENGTH}, S=${CLIP_STRIDE}, deform_scale=${DEFORMABLE_ALIGNMENT_SCALE}"
...
echo "Combined V6 evaluation complete: ${OUTPUT_DIR}/summary.json"
